...
from models.recurrent_models import recurrent_model, nonrecurrent_model
...
class EddlRecurrentModule:
    ...
    def create_model(self, visual_dim, semantic_dim, rec_cell_type="lstm", for_predictions=False):
        # cnn
        vs = self.voc_size  # vocabulary size
        if not for_predictions:
            dropout = self.conf.get("dropout", 0.2)
            print("using dropout:", dropout)
            model = recurrent_model(visual_dim, semantic_dim, vs, self.conf.emb_size, self.conf.lstm_size, dropout_rate=dropout, rec_cell_type=self.conf.rec_cell_type)
        else:
            assert self.rnn is not None
            model = nonrecurrent_model(visual_dim, semantic_dim, vs, self.conf.emb_size, self.conf.lstm_size, rec_cell_type=self.conf.rec_cell_type)
        #<
    ...
    def build_model(self, mdl=None, for_predictions=False):
        ...
        if mdl is None:
            ...
            print(f"creating model with rec cell type: {self.conf.rec_cell_type}")         
            rnn = self.create_model(visual_dim, semantic_dim, rec_cell_type=self.conf.rec_cell_type, for_predictions=for_predictions)
    ...
    def train(self):
        ...
        for ei in range(n_epochs):
            ...
            print(f"avg time per epoch: {np.mean(self.timings)} seconds, {H.precisedelta(np.mean(self.timings))}")
            if self.conf.remote_log:
                self.run[f"timings/avg_epoch"] = H.precisedelta(np.mean(self.timings))
            ...
            print("---> exp folder: ", self.exp_fld)
            print(self.conf.get("description", "na"))
            print("")
    ...
    def save_checkpoint(self, filename="rec_checkpoint"):
        ...
        # eddl.save_net_to_onnx_file(self.rnn, join(self.fld, filename + ".onnx"), seq_len=self.seq_len)
        # eddl.save_net_to_onnx_file(self.rnn, join(self.fld, filename + ".onnx"), self.seq_len)
        eddl.save_net_to_onnx_file(self.rnn, join(self.fld, filename + ".onnx"), seq_len=self.seq_len)
