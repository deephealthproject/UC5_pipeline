...
def recurrent_model(visual_dim, semantic_dim, vs, emb_size, lstm_size, init_v=0.05, dropout_rate=0.2, rec_cell_type="lstm"):
    if rec_cell_type == "lstm":
        return recurrent_lstm_model(visual_dim, semantic_dim, vs, emb_size, lstm_size, init_v, dropout_rate)
    elif rec_cell_type == "gru":
        return recurrent_gru_model(visual_dim, semantic_dim, vs, emb_size, lstm_size, init_v, dropout_rate)
    else:
        assert False, f"Unknown cell type: {rec_cell_type}"
    
def recurrent_lstm_model(visual_dim, semantic_dim, vs, emb_size, lstm_size, init_v=0.05, dropout_rate=0.2):
    assert init_v > 0
    ...
    to_lstm = eddl.Dropout(to_lstm, rate=dropout_rate)
...
def nonrecurrent_model(visual_dim, semantic_dim, vs, emb_size, lstm_size, init_v=0.05, dropout_rate=0.2, rec_cell_type="lstm"):
    if rec_cell_type == "lstm":
        return nonrecurrent_lstm_model(visual_dim, semantic_dim, vs, emb_size, lstm_size, init_v)
    elif rec_cell_type == "gru":
        return nonrecurrent_gru_model(visual_dim, semantic_dim, vs, emb_size, lstm_size, init_v)
    else:
        assert False, f"Unknown cell type: {rec_cell_type}"
...
def recurrent_gru_model(visual_dim, semantic_dim, vs, emb_size, gru_size, init_v=0.05, dropout_rate=0.2):
    assert init_v > 0
    cnn_top_in, cnn_out_in, context = _shared_top(visual_dim, semantic_dim, emb_size, init_v)
    # lstm
    word_in = eddl.Input([vs])
    # to_lstm but actually to gru
    to_lstm = eddl.ReduceArgMax(word_in, [0])
    to_lstm = eddl.RandomUniform(eddl.Embedding(to_lstm, vs, 1, emb_size, mask_zeros=True, name="word_embs"), -init_v, init_v)
    to_lstm = eddl.Concat([to_lstm, context])
    ...
    lstm = eddl.GRU(to_lstm, gru_size, mask_zeros=True, bidirectional=False, name="gru_cell")
    eddl.setDecoder(word_in)
    out_lstm = eddl.Softmax(eddl.Dense(lstm, vs, name="out_dense"), name="rnn_out")
    print(f"GRU recurrent module, output shape: {out_lstm.output.shape}")
    # model
    rnn = eddl.Model([cnn_top_in, cnn_out_in], [out_lstm])
    eddl.summary(rnn)
    return rnn
...
# model used for generating text (test-inference stage)
def nonrecurrent_gru_model(visual_dim, semantic_dim, vs, emb_size, gru_size, init_v=0.05):
    cnn_top_in, cnn_out_in, context = _shared_top(visual_dim, semantic_dim, emb_size, init_v)
    
    lstm_in = eddl.Input([vs])
    lstate = eddl.States([1, gru_size])
    
    to_rec = eddl.ReduceArgMax(lstm_in, [0])  # word index
    to_rec = eddl.Embedding(to_rec, vs, 1, emb_size, name="word_embs")
    to_rec = eddl.Concat([to_rec, context])
    gru = eddl.GRU([to_rec, lstate], gru_size, True, name="gru_cell")
    gru.isrecurrent = False
    
    out_gru = eddl.Softmax(
                eddl.Dense(gru, vs, name="out_dense"), 
                name="rnn_out")
    
    # *** model
    model = eddl.Model([cnn_top_in, cnn_out_in, lstm_in, lstate], [out_gru])
    # if the model is saved in onnx, there is the same error as in the recurrent model when loaded: 
    #       LDense only works over 2D tensors (LDense)
    return model
...
def generate_text_predict_next_gru(rnn, target_text, n_tokens, visual_batch=None, semantic_batch=None, dev=False):
    assert (visual_batch is not None) and (semantic_batch is not None)

    bs = visual_batch.shape[0]
    gru = eddl.getLayer(rnn, "gru_cell")
    lstm_size = gru.output.shape[1]
    last_layer = eddl.getLayer(rnn, "rnn_out")
    voc_size = last_layer.output.shape[1]
    ...
    # lstm cell states
    state_t = Tensor.zeros([bs, 1, lstm_size])
    ...
    for j in range(0, n_tokens):
        ...
        # forward: token and state_t update after the forward step
        eddl.forward(rnn, [visual_batch, semantic_batch, token, state_t])     
        states = eddl.getStates(gru)
        # print("GRU getStates() dimension: ", len(states))
...
# -----------------------------------------------------------------------------------
def recurrent_lstm_model_2(visual_dim, semantic_dim, vs, emb_size, lstm_size, init_v=0.05, n_lstms=1):
    assert init_v > 0
    cnn_top_in, cnn_out_in, context = _shared_top(visual_dim, semantic_dim, emb_size, init_v)
    # lstm
    word_in = eddl.Input([vs])
    ...
    to_lstm = eddl.RandomUniform(eddl.Embedding(to_lstm, vs, 1, emb_size, mask_zeros=True, name="word_embs"), -init_v, init_v)
    to_lstm = eddl.Concat([to_lstm, context])
    lstm = eddl.LSTM(to_lstm, lstm_size, mask_zeros=True, bidirectional=False, name="lstm_cell0")
    for i in range(1, n_lstms):
        lstm = eddl.LSTM(lstm, lstm_size, mask_zeros=True, bidirectional=False, name=f"lstm_cell{i}")
    eddl.setDecoder(word_in)
    out_lstm = eddl.Softmax(eddl.Dense(lstm, vs, name="out_dense"), name="rnn_out")
    print(f"layer lstm, output shape: {out_lstm.output.shape}")
    # model
...
# model used for generating text (test-inference stage)
def nonrecurrent_lstm_model_2(visual_dim, semantic_dim, vs, emb_size, lstm_size, init_v=0.05):
    cnn_top_in, cnn_out_in, context = _shared_top(visual_dim, semantic_dim, emb_size, init_v)
