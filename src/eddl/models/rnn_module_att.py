...
class EddlRecurrentModuleWithAttn:
    ...
    def init_neptune(self):
        ...
        print("NEPTUNE descr:",self.conf.get("description", "na"))
        run["description"] = self.conf.get("description", "na")
        return run 
    #<
    ...
    def create_model_attn(self, visual_dim, semantic_dim, for_predictions=False, n_lstm_cells=1, init_v=0.05):
        ...
        # cnn_concat = eddl.Concat([cnn_top_in, cnn_out_in], name="co_attention") 
        
        # please notice: in the current implementation LSTM_SIZE must be equale to EMB_SIZE
        attn_lstm_h_in = eddl.Input([self.conf.lstm_size], name="in_lstm_hidden")
        lstate = eddl.States([2, self.conf.lstm_size])

        W_v = eddl.RandomUniform(eddl.Dense(cnn_top_in, visual_dim, name="W_v"), -init_v, init_v)
        W_v_h = eddl.RandomUniform(eddl.Dense(attn_lstm_h_in, visual_dim, name="W_v_h"), -init_v, init_v)
        v_tanh = eddl.Tanh(eddl.Add([W_v, W_v_h], name="v_attn_tanh"))
        W_v_attn = eddl.RandomUniform(eddl.Dense(v_tanh, visual_dim), -init_v, init_v)
        alpha_v = eddl.Softmax(W_v_attn)
        v_att = eddl.Mult(alpha_v, cnn_out_in)
        print("VISUAL ATTENTION, layers created")

        W_s = eddl.RandomUniform(eddl.Dense(cnn_out_in, semantic_dim, name="W_s"), -init_v, init_v)
        W_s_h = eddl.RandomUniform(eddl.Dense(attn_lstm_h_in, semantic_dim, name="W_s_h"), -init_v, init_v)
        s_tanh = eddl.Tanh(eddl.Add([W_s, W_s_h], name="s_attn_tanh"))
        W_s_attn = eddl.RandomUniform(eddl.Dense(s_tanh, semantic_dim), -init_v, init_v)
        alpha_s = eddl.Softmax(W_s_attn)
        s_att = eddl.Mult(alpha_s, cnn_top_in)
        print("SPATIAL ATTENTION, layers created")
        print("Visual attention, output shape:", v_att.output.shape)
        print("Spatial attention, output shape:", s_att.output.shape)

        concatenation = eddl.Concat([v_att, s_att])
        print("concatenation, output shape", concatenation.output.shape)
        context = eddl.RandomUniform( eddl.Dense( concatenation, self.conf.emb_size, name="context" ) , -init_v, init_v) 
        ...
        lstm = eddl.LSTM([to_lstm, lstate], self.conf.lstm_size, mask_zeros=True, bidirectional=False, name="lstm0")
        lstm.isrecurrent = False

        lstms = []
        # lstms.add(lstm)
        # for i in range(1, n_lstm_cells):
        #     lstm = eddl.LSTM(lstm, self.conf.lstm_size, mask_zeros=True, bidirectional=False, name=f"lstm{i}")
        #     lstms.add(lstm)
        #  self.lstms = lstms
        ...
        # model
        rnn = eddl.Model([cnn_top_in, cnn_out_in, word_in, attn_lstm_h_in, lstate], [out_lstm])
        print("RNN WITH ATTN CREATED WITH SUCCESS")
        return rnn
    ...
    def build_model(self, mdl=None, for_predictions=False):
        ...
        if mdl is None:
            ...
            rnn = self.create_model_attn(visual_dim, semantic_dim, for_predictions=for_predictions)
    ...
    def train(self):
        ...
        lstm = eddl.getLayer(self.rnn, "lstm0")
        last_layer = eddl.getLayer(self.rnn, "rnn_out")
        ...
        if self.conf.remote_log:
            self.conf["gpus"] = self.conf.gpu_id
        ...
        for ei in range(n_epochs):
            ...
            for stage in self.stages.keys():
                ...
                print(f"> STAGE: {stage}, loss reset")
                ...
                for bi in range(ds.GetNumBatches()):
                    ...
                    if (bi + 1) % 100 == 0:
                        print(f"batch {bi+1} / {ds.GetNumBatches()}")
                        print(f"time since beginning of stage {stage}: {H.precisedelta(time.perf_counter() - stage_t0)}")
                    ...
                    if self.seq_len is None:
                        self.seq_len = texts.shape[1]
                        print("SEQ_LEN for onnx export set to:", self.seq_len)
                    ...
                    state_t = Tensor.zeros([Y.shape[0], 2, lstm_size])
                    token = Tensor.zeros([Y.shape[0], self.voc_size])
                    # for over target tokens
                    for j in range(1, Y.shape[1]):
                        token = Y.select([":", str(j-1), ":"]).squeeze()
                        print(f"TOKEN {j-1} out of {Y.shape[1]}")
                        # [cnn_top_in, cnn_out_in, word_in, attn_lstm_h_in, lstate]
                        self.rnn.forward([cnn_semantic, cnn_visual, token, state_t.select([":",  "0", ":"]), state_t])
                    ...
                    if self.dev and bi == 5:
                        ...
                        break
                #< for over batches
                print(f"stage {stage} completed in {H.precisedelta(time.perf_counter() - stage_t0)}")
                ds.Stop()
            ...
            if self.conf.remote_log:
                self.run[f"timings/avg_epoch"] = H.precidedelta(np.mean(self.timings))
    ...
    def inner_forward(self, cnn_visual, cnn_semantic, texts):
        ...
        lstm = eddl.getLayer(self.rnn, "lstm_cell0")
        lstm_size = lstm.output.shape[1]
        n_lstms = len(self.lstms)
        print(f"number of stacked lstm cells: {n_lstms}")

         # lstm cell states
        state_tensors = [Tensor.zeros([bs, 2, lstm_size])  for i in range(n_lstms)]

        for i in range(1, Y.shape[1]):
            bws = Y[":", str(i), ":"]
            bws = bws.reshape(bws.shape[0], bws.shape[2])

            print("up to word at index:", i)        
            
            eddl.forward(self.rnn, [cnn_visual, cnn_semantic, word_i])
        # for i over words
    #< inner_forward
