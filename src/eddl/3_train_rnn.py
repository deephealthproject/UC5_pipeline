...
from models.rnn_module import EddlRecurrentModule
import fire
...
def load_ecvl_dataset(filename, config):
    ...
    if config["eddl_cs"] == "cpu":
        ...
        num_workers = 4 if nnz(config["gpu_id"]) == 1 else 8 * nnz(config["gpu_id"])
...
def do_training(exp_fld, fld, dataset, text_ds, config):
    rec_module = EddlRecurrentModule(dataset, text_ds, exp_fld, fld, config)
...
def main(exp_fld, 
    ...
    print("RECURRENT LAYER: ", rec_cell_type)
    ...
    for fld in run_flds:
        print(f"\t{fld}")
    print()
    for fld in sorted(run_flds, reverse=True):
        ...
        for param_fld in sorted(params_flds):
